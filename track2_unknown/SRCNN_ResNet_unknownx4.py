...
def write_example(HR_paths, LR_paths, model, exp_index=None, write_path=r"/home/uceejm3/exp/"):
    if exp_index == None:
        exp_index = PSNR_list.index(max(PSNR_list))
    ...
    for i in exp_index:
        ...
        cv2.imwrite(write_path+"LR_X4_"+str(i)+"_PSNR"+str(PSNR_BIC)+"_SSIM"+str(SSIM_BIC)+".jpg", LR_BIC*255.)
        cv2.imwrite(write_path+"SR_X4_FDSRCNN_"+str(i)+"_PSNR"+str(PSNR_SR)+"_SSIM"+str(SSIM_SR)+".jpg",SR_exp*255.)   
...
def loss(y_true, y_pred):
    # mse = K.mean(K.square(y_pred - y_true), axis=[-1,-2])
    HR = tf.convert_to_tensor(y_true)
    SR = tf.convert_to_tensor(y_pred)
    ssim = tf.reduce_mean(tf.image.ssim(SR,HR,max_val=1.0))
    psnr = tf.reduce_mean(tf.image.psnr(SR,HR,max_val=1.0))
    return -((psnr)/35 - ssim)

#%%
trainVal_HR_paths = glob.glob(r"/home/uceejm3/AMLS2/project/processed_ds/training/HR/*")
trainVal_unknown4_paths = glob.glob(r"/home/uceejm3/AMLS2/project/processed_ds/training/unknown/X4/*")
...
train_unknown4_paths = trainVal_unknown4_paths[:train_size]
...
X4path_tr_ds = tf.data.Dataset.from_tensor_slices(train_unknown4_paths)
X4_tr_ds = X4path_tr_ds.map(load_image, num_parallel_calls=AUTOTUNE)

train_ds = tf.data.Dataset.zip((X4_tr_ds,HR_tr_ds))
...
val_unknown4_paths = trainVal_unknown4_paths[-val_size:]
...
X4path_val_ds = tf.data.Dataset.from_tensor_slices(val_unknown4_paths)
X4_val_ds = X4path_val_ds.map(load_image, num_parallel_calls=AUTOTUNE)

val_ds = tf.data.Dataset.zip((X4_val_ds,HR_val_ds))
val_ds = val_ds.cache().shuffle(buffer_size=val_size, seed=17).batch(batch_size)
val_ds = val_ds.prefetch(buffer_size=AUTOTUNE) 

fdsrcnn = FD_SRCNN()
fdsrcnn = multi_gpu_model(fdsrcnn, gpus=gpu_num)   
fdsrcnn.compile(optimizer = 'adam' ,loss = ['mse'],metrics=[PSNR_cal,SSIM_cal])
steps=tf.math.ceil(train_size/batch_size).numpy()

def scheduler(epoch):
     if epoch % 10 == 0 and epoch != 0:
         lr = K.get_value(fdsrcnn.optimizer.lr)
         K.set_value(fdsrcnn.optimizer.lr, lr * 0.5)
         print("lr changed to {}".format(lr * 0.5))
     return K.get_value(fdsrcnn.optimizer.lr)
...
fdsrcnn.fit(train_ds, epochs=epochs, steps_per_epoch=int(steps) , callbacks=[reduce_lr,early_stopping], verbose=2, validation_data=val_ds)

#%% Model save and load
fdsrcnn.save(r"/home/uceejm3/AMLS2/project/saved_models/unknown/FDSRCNN_unknownX4.h5")
print("model has saved")

from tensorflow.keras.models import load_model
fdsrcnn = load_model(r'/home/uceejm3/AMLS2/project/good_models/unknown/FDSRCNN_unknownX4.h5',custom_objects={'PSNR_cal': PSNR_cal, 'SSIM_cal':SSIM_cal})
print("model loaded!")
...
trainVal_unknown4_eval_paths = sorted(glob.glob(r"/home/uceejm3/AMLS2/project/dataset/training_set/unknown/X4/*"), reverse=True)
...
train_unknown4_paths = trainVal_unknown4_eval_paths[:train_eval_size]
train_PSNR, train_SSIM, train_PSNR_list, train_SSIM_list = evaluation(train_HR_eval_paths,train_unknown4_paths,model=fdsrcnn)
...
val_unknown4_paths = trainVal_unknown4_eval_paths[-val_eval_size:]
val_PSNR, val_SSIM, val_PSNR_list, val_SSIM_list = evaluation(val_HR_eval_paths,val_unknown4_paths,model=fdsrcnn)
...
test_unknown4_paths = sorted(glob.glob(r"/home/uceejm3/AMLS2/project/dataset/test_set/unknown/X4/*"), reverse=True)

test_PSNR,test_SSIM, PSNR_list, SSIM_list = evaluation(test_HR_paths,test_unknown4_paths,model=fdsrcnn)
exp_index=[22,61,79]
write_example(test_HR_paths, test_unknown4_paths, model=fdsrcnn, exp_index=exp_index, write_path=r"/home/uceejm3/exp/")
